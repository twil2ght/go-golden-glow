...
func (reg *registry) Register(pluginName string, method Handler) error {
	...
	logger.Info("extractor:register successfully", "plugin", pluginName)
	return nil
}
...
func (reg *registry) OnRegisterNodeRegistry(nReg node.Registry) error {
	...
	logger.Info("extractor:register to node registry", "handler_amount", len(reg.handlers))
	return nReg.Register(KeyExtractor, defaultCreator)
}
